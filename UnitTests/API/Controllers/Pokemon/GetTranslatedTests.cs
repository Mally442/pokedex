...
{
    /// <summary>
    /// Tests for the PokemonController GetTranslated action
    /// </summary>
    [TestFixture]
    public class GetTranslatedTests
    {
        ...
        public async Task WhenServiceReturnsNull()
        {
            ...
            // Act
            var response = await _controller.GetTranslated("mewtwo");
        ...
        public async Task WhenServiceReturnsSpecies()
        {
            ...
            _speciesService.Setup(e => e.Get(It.IsAny<string>(), true))
                .Returns(Task.FromResult(species));

            // Act
            var response = await _controller.GetTranslated("mewtwo") as OkObjectResult;
            var responseValue = response.Value as Species;

            // Assert
            Assert.AreEqual("translated description", responseValue.Description);
        }
